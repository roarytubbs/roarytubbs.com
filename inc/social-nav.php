...
class rt_Social {
	...
	private function init() {
		$this->social_networks = array(
			...
			'codepen.io'        => array( 'name' => 'Codepen',        'class' => 'codepen',       'icon' => 'fa-codepen',       'icon-sign' => 'fa-codepen'          ),
			...
			foreach( $urls as $index => $url_data ) {
				...
				foreach( $this->social_networks as $domain => $args ) {
					if ( false !== strpos( $url, $domain ) ) {
						$html .= sprintf( '<li><a href="%1$s" title="%2$s"><i class="fa fa-%2$s fa-fw"></i><span class="fa-content">&nbsp;%3$s</span></a></li>',esc_url( $url ), esc_attr( $args[ 'class' ] ), esc_html( $placeholder ) );
					}
				}
			}
			$html .= '</ul>';
	...
	function rt_get_social( $echo = true ) {
